...
status_t ipc_serialize_heartbeat(const ipc_heartbeat_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
status_t ipc_deserialize_heartbeat(ipc_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    // Validasi Pointer Input
    if (!p || !buffer || !offset_ptr || !p->payload.ipc_heartbeat) {
        fprintf(stderr, "[ipc_deserialize_heartbeat Error]: Invalid input pointers.\n");
        return FAILURE;
    }

    size_t current_offset = *offset_ptr; // Ambil offset dari pointer input
    const uint8_t *cursor = buffer + current_offset;
    ipc_heartbeat_t *payload = p->payload.ipc_heartbeat; // Payload spesifik yang akan diisi (STRUCT DENGAN FAM)

    fprintf(stderr, "==========================================================Panjang offset_ptr AWAL: %ld\n", (long)(cursor - buffer));

    // 1. Deserialisasi wot
    if (current_offset + 1 > total_buffer_len) {
        fprintf(stderr, "[ipc_deserialize_heartbeat Error]: Out of bounds reading wot.\n");
    ...
    // 2. Deserialisasi index
    if (current_offset + 1 > total_buffer_len) {
        fprintf(stderr, "[ipc_deserialize_heartbeat Error]: Out of bounds reading index.\n");
        return FAILURE_OOBUF;
    }
...
ipc_protocol_t_status_t ipc_prepare_cmd_heartbeat(int *fd_to_close, worker_type_t wot, uint8_t index) {
	...
	if (!payload) {
		perror("Failed to allocate ipc_heartbeat_t payload");
		CLOSE_FD(fd_to_close);
