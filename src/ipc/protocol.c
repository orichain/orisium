...
ssize_t_status_t ipc_serialize(const char *label, const ipc_protocol_t* p, uint8_t** ptr_buffer, size_t* buffer_size) {
    ...
    switch (p->type) {
        case IPC_CLIENT_REQUEST_TASK:
            result_pyld = ipc_serialize_client_request_task(label, p->payload.ipc_client_request_task, current_buffer, *buffer_size, &offset);
            break;
        case IPC_CLIENT_DISCONNECTED:
            result_pyld = ipc_serialize_client_disconnect_info(label, p->payload.ipc_client_disconnect_info, current_buffer, *buffer_size, &offset);
            break;
        case IPC_SHUTDOWN:
            result_pyld = ipc_serialize_shutdown(label, p->payload.ipc_shutdown, current_buffer, *buffer_size, &offset);
            break;
        case IPC_HEARTBEAT:
            result_pyld = ipc_serialize_heartbeat(label, p->payload.ipc_heartbeat, current_buffer, *buffer_size, &offset);
            break;
        default:
            LOG_ERROR("%sUnknown protocol type for serialization: 0x%02x", label, p->type);
            result.status = FAILURE_IPYLD;
    ...
    switch (p->type) {
        ...
        case IPC_CLIENT_DISCONNECTED: {
            ...
            if (!disconnect_info_payload) {
                LOG_ERROR("%sFailed to allocate ipc_client_disconnect_info_t without FAM. %s", label, strerror(errno));
                CLOSE_IPC_PROTOCOL(&p);
                result.status = FAILURE_NOMEM;
                return result;
            }
            p->payload.ipc_client_disconnect_info = disconnect_info_payload;
            result_pyld = ipc_deserialize_client_disconnect_info(label, p, buffer, len, &current_buffer_offset);
            break;
		}
		case IPC_SHUTDOWN: {
            if (current_buffer_offset + fixed_header_size > len) {
