...
status_t ipc_serialize_client_disconnect_info(const char *label, const ipc_client_disconnect_info_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
status_t ipc_deserialize_client_disconnect_info(const char *label, ipc_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    ...
    ipc_client_disconnect_info_t *payload = p->payload.ipc_client_disconnect_info;
...
ipc_protocol_t_status_t ipc_prepare_cmd_client_disconnect_info(const char *label, int *fd_to_close, uint8_t disconnected_client_ip[]) {
	...
	if (!payload) {
        LOG_ERROR("%sFailed to allocate ipc_client_disconnect_info_t payload. %s", label, strerror(errno));
		CLOSE_FD(fd_to_close);
