...
status_t setup_dbr_worker(dbr_context_t *dbr_ctx, worker_type_t wot, int worker_idx, long initial_delay_ms, int master_uds_fd) {
    if (setup_worker(&dbr_ctx->worker, "DBR", wot, worker_idx, initial_delay_ms, master_uds_fd) != SUCCESS) return FAILURE;
    return SUCCESS;
}

void run_dbr_worker(worker_type_t wot, int worker_idx, long initial_delay_ms, int master_uds_fd) {
    dbr_context_t dbr_ctx;
    if (setup_dbr_worker(&dbr_ctx, wot, worker_idx, initial_delay_ms, master_uds_fd) != SUCCESS) goto exit;	    
