...
//----------------------------------------------------------------------
                ...
                if (check_mac_ctr(
                        dbr_ctx.worker.label, 
                        dbr_ctx.worker.aes_key, 
                        dbr_ctx.worker.mac_key, 
                        &dbr_ctx.worker.remote_ctr, 
				...
				if (ircvdi.r_ipc_raw_protocol_t->type == IPC_MASTER_WORKER_INFO) {
                    ipc_protocol_t_status_t deserialized_ircvdi = ipc_deserialize(dbr_ctx.worker.label,
                        dbr_ctx.worker.aes_key, dbr_ctx.worker.remote_nonce, &dbr_ctx.worker.remote_ctr,
...
//----------------------------------------------------------------------
                        if (initial_delay_ms > 0) {
                            LOG_DEBUG("%sApplying initial delay of %ld ms...", dbr_ctx.worker.label, initial_delay_ms);
...
//----------------------------------------------------------------------
                    ...
                    kdf1(dbr_ctx.worker.kem_sharedsecret, dbr_ctx.worker.aes_key);
                    kdf2(dbr_ctx.worker.aes_key, dbr_ctx.worker.mac_key);
