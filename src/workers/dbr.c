...
void cleanup_dbr_worker(worker_context_t *ctx) {
	cleanup_worker(ctx);
}

status_t setup_dbr_worker(worker_context_t *ctx, worker_type_t *wot, uint8_t *index, int *master_uds_fd) {
    if (setup_worker(ctx, "DBR", wot, index, master_uds_fd) != SUCCESS) return FAILURE;
    return SUCCESS;
}
