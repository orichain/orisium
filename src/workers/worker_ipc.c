...
//---------------------------------------------------------------------- 
        ...
        status_t chst = oritw_add_event(&worker_ctx->timer, &worker_ctx->heartbeat_timer_id.event, worker_ctx->heartbeat_timer_id.id, delay_ms);
        if (chst != SUCCESS) {
...
//----------------------------------------------------------------------
    uint64_t_status_t current_time = get_monotonic_time_ns(worker_ctx->label);
    if (current_time.status != SUCCESS) {
        memset(control_packet_ack->udp_data.r_puint8_t, 0, control_packet_ack->udp_data.r_size_t);
        free(control_packet_ack->udp_data.r_puint8_t);
        return FAILURE;
    }
...
//----------------------------------------------------------------------
    ...
    )
    {
        ...
        size_t data_4mac_len = control_packet_ack->udp_data.r_size_t - AES_TAG_BYTES;
        uint8_t *data_4mac = control_packet_ack->udp_data.r_puint8_t + data_4mac_offset;
        uint8_t mac[AES_TAG_BYTES];
        calculate_mac(security->mac_key, data_4mac, mac, data_4mac_len);
        memcpy(control_packet_ack->udp_data.r_puint8_t, mac, AES_TAG_BYTES);
    } else {
...
//----------------------------------------------------------------------
    uint64_t_status_t current_time = get_monotonic_time_ns(worker_ctx->label);
    if (current_time.status != SUCCESS) {
        memset(control_packet->udp_data.r_puint8_t, 0, control_packet->udp_data.r_size_t);
...
//----------------------------------------------------------------------
    ...
    {
        const size_t data_4mac_offset = AES_TAG_BYTES;
        size_t data_4mac_len = control_packet->udp_data.r_size_t - AES_TAG_BYTES;
        uint8_t *data_4mac = control_packet->udp_data.r_puint8_t + data_4mac_offset;
        uint8_t mac[AES_TAG_BYTES];
        calculate_mac(security->mac_key, data_4mac, mac, data_4mac_len);
        memcpy(control_packet->udp_data.r_puint8_t, mac, AES_TAG_BYTES);
    } else {
        uint8_t rendom_mac[AES_TAG_BYTES];
        generate_fast_salt(rendom_mac, AES_TAG_BYTES);
...
//----------------------------------------------------------------------
    if (isretry) {
        if (session->hello4_ack.udp_data.r_puint8_t != NULL) {
...
//======================================================================
    cleanup_control_packet_ack(&session->hello4_ack, false, CDT_FREE);
    uint64_t_status_t current_time = get_monotonic_time_ns(worker_ctx->label);
    if (current_time.status != SUCCESS) {
        CLOSE_IPC_PROTOCOL(&received_protocol);
        CLOSE_ORILINK_PROTOCOL(&received_orilink_protocol);
...
//----------------------------------------------------------------------
    if (isretry) {
        if (session->hello3_ack.udp_data.r_puint8_t != NULL) {
...
//======================================================================
    cleanup_control_packet_ack(&session->hello3_ack, false, CDT_FREE);
    uint64_t_status_t current_time = get_monotonic_time_ns(worker_ctx->label);
    if (current_time.status != SUCCESS) {
        CLOSE_IPC_PROTOCOL(&received_protocol);
        CLOSE_ORILINK_PROTOCOL(&received_orilink_protocol);
...
//----------------------------------------------------------------------
    if (isretry) {
        if (session->hello2_ack.udp_data.r_puint8_t != NULL) {
...
//======================================================================
    cleanup_control_packet_ack(&session->hello2_ack, false, CDT_FREE);
    uint64_t_status_t current_time = get_monotonic_time_ns(worker_ctx->label);
    if (current_time.status != SUCCESS) {
        CLOSE_IPC_PROTOCOL(&received_protocol);
        CLOSE_ORILINK_PROTOCOL(&received_orilink_protocol);
...
//----------------------------------------------------------------------
    if (isretry) {
        if (session->hello1_ack.udp_data.r_puint8_t != NULL) {
...
//======================================================================
    cleanup_control_packet_ack(&session->hello1_ack, false, CDT_FREE);
    uint64_t_status_t current_time = get_monotonic_time_ns(worker_ctx->label);
    if (current_time.status != SUCCESS) {
        CLOSE_IPC_PROTOCOL(&received_protocol);
        CLOSE_ORILINK_PROTOCOL(&received_orilink_protocol);
...
//======================================================================
                status_t chst = oritw_add_event(&worker_ctx->timer, &session->hello1.retry_timer_id.event, session->hello1.retry_timer_id.id, retry_timer_interval);
                if (chst != SUCCESS) {
                    CLOSE_IPC_PROTOCOL(&received_protocol);
                    return FAILURE;
                }
//======================================================================
        ...
        case ORILINK_HELLO2: {
            if (iudp_data_acki->trycount == (uint8_t)1) {
//======================================================================
                status_t chst = oritw_add_event(&worker_ctx->timer, &session->hello2.retry_timer_id.event, session->hello2.retry_timer_id.id, retry_timer_interval);
                if (chst != SUCCESS) {
                    CLOSE_IPC_PROTOCOL(&received_protocol);
                    return FAILURE;
                }
//======================================================================
        ...
        case ORILINK_HELLO3: {
            if (iudp_data_acki->trycount == (uint8_t)1) {
//======================================================================
                status_t chst = oritw_add_event(&worker_ctx->timer, &session->hello3.retry_timer_id.event, session->hello3.retry_timer_id.id, retry_timer_interval);
                if (chst != SUCCESS) {
                    CLOSE_IPC_PROTOCOL(&received_protocol);
                    return FAILURE;
                }
//======================================================================
        ...
        case ORILINK_HELLO4: {
            if (iudp_data_acki->trycount == (uint8_t)1) {
//======================================================================
                status_t chst = oritw_add_event(&worker_ctx->timer, &session->hello4.retry_timer_id.event, session->hello4.retry_timer_id.id, retry_timer_interval);
                if (chst != SUCCESS) {
                    CLOSE_IPC_PROTOCOL(&received_protocol);
                    return FAILURE;
                }
...
//----------------------------------------------------------------------
                ...
                status_t otmr = oritw_add_event(&worker_ctx->timer, &session->heartbeat_openner_timer_id.event, session->heartbeat_openner_timer_id.id, hb_interval);
                if (otmr != SUCCESS) {
                    return FAILURE;
                }
//======================================================================
                status_t chst = oritw_add_event(&worker_ctx->timer, &session->heartbeat.retry_timer_id.event, session->heartbeat.retry_timer_id.id, retry_timer_interval);
                if (chst != SUCCESS) {
                    CLOSE_IPC_PROTOCOL(&received_protocol);
                    return FAILURE;
                }
...
//======================================================================
                status_t chst = oritw_add_event(&worker_ctx->timer, &session->heartbeat_sender_timer_id.event, session->heartbeat_sender_timer_id.id, timer_interval);
                if (chst != SUCCESS) {
                    CLOSE_IPC_PROTOCOL(&received_protocol);
                    return FAILURE;
                }
