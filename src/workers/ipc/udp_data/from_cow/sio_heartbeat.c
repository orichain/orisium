...
//======================================================================
    //LOG_DEVEL_DEBUG("Hereeeeeeeeeeeeeeeeeeeee....... cow_heartbeat.c create_heartbeat_sender_timer_fd FD %d\n", session->heartbeat_sender_timer_fd);
...
//======================================================================
    if (session->is_first_heartbeat) {
        if (*trycount > (uint8_t)1) {
            ...
            reset_packet_ack_data(&session->hello4_ack);
            
            LOG_DEVEL_DEBUG("%sRTT Hello-4 Ack = %f\n", worker_ctx->label, session->rtt.value_prediction);
        }
    } else {
//======================================================================
...
//======================================================================
    ...
    )
    {
        ...
        LOG_DEVEL_DEBUG("%sRetry Detected\n", worker_ctx->label);
    } else {
...
//======================================================================
    ...
    {
        LOG_DEVEL_DEBUG("[Debug Here Helper]: Heartbeat Ack Packet Number %d. Sending To Fake Addr To Force Retry\n", session->test_drop_heartbeat_ack);
