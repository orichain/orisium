...
//======================================================================
    ...
    uint64_t intv_rcvd = current_time_rcvd.r_uint64_t - session->hello4_ack.last_receive;
    double intv_rcvd_value = (double)intv_rcvd;
    if (intv_rcvd_value < (double)1) {
        LOG_ERROR("%sHello4 Burst Detected. Ignoring.", worker_ctx->label);
        CLOSE_IPC_PROTOCOL(&received_protocol);
        CLOSE_ORILINK_RAW_PROTOCOL(&oudp_datao);
        return FAILURE;
    }
    session->hello4_ack.last_receive = current_time_rcvd.r_uint64_t;
//======================================================================
    if (!isretry) {
...
//----------------------------------------------------------------------
    if (isretry) {
        if (session->hello4_ack.data != NULL) {
            ...
            if (retry_control_packet_ack(worker_ctx, identity, security, &session->hello4_ack) != SUCCESS) {
                CLOSE_IPC_PROTOCOL(&received_protocol);
                CLOSE_ORILINK_RAW_PROTOCOL(&oudp_datao);
                return FAILURE;
            }
