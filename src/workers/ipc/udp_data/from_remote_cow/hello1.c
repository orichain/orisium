...
//======================================================================
    ...
    uint64_t intv_rcvd = current_time_rcvd.r_uint64_t - session->hello1_ack.last_receive;
    double intv_rcvd_value = (double)intv_rcvd;
    if (intv_rcvd_value < (double)1) {
        LOG_ERROR("%sHello1 Burst Detected. Ignoring.", worker_ctx->label);
        CLOSE_IPC_PROTOCOL(&received_protocol);
        CLOSE_ORILINK_RAW_PROTOCOL(&oudp_datao);
        return FAILURE;
    }
    session->hello1_ack.last_receive = current_time_rcvd.r_uint64_t;
//======================================================================
