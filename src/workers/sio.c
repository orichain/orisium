...
//----------------------------------------------------------------------
				...
				if (ircvdi.r_ipc_raw_protocol_t->type == IPC_MASTER_WORKER_INFO) {
                    ...
                    if (deserialized_ircvdi.status != SUCCESS) {
                        ...
                        LOG_DEBUG("%sipc_deserialize BERHASIL.", sio_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                    }           
                    ...
                    if (iinfoi->flag == IT_SHUTDOWN) {
                        ...
                        LOG_INFO("%sMaster Ready ...", sio_ctx.worker.label);
//----------------------------------------------------------------------
                        if (initial_delay_ms > 0) {
                            LOG_DEVEL_DEBUG("%sApplying initial delay of %ld ms...", sio_ctx.worker.label, initial_delay_ms);
                            sleep_ms(initial_delay_ms);
                        }
//----------------------------------------------------------------------
                        if (worker_master_hello1(&sio_ctx.worker) != SUCCESS) {
                            LOG_ERROR("%sWorker error. Initiating graceful shutdown...", sio_ctx.worker.label);
                            sio_ctx.worker.shutdown_requested = 1;
                            CLOSE_IPC_PROTOCOL(&received_protocol);
                            continue;
                        }
                    ...
                    if (!sio_ctx.worker.hello1_sent) {
                        LOG_ERROR("%sBelum pernah mengirim HELLO1", sio_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
                    if (sio_ctx.worker.hello1_ack_rcvd) {
                        LOG_ERROR("%sSudah ada HELLO1_ACK", sio_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
					ipc_protocol_t_status_t deserialized_ircvdi = ipc_deserialize(sio_ctx.worker.label,
                    ...
                    memcpy(sio_ctx.worker.kem_ciphertext, ihello1_acki->kem_ciphertext, KEM_CIPHERTEXT_BYTES);
                    if (KEM_DECODE_SHAREDSECRET(sio_ctx.worker.kem_sharedsecret, sio_ctx.worker.kem_ciphertext, sio_ctx.worker.kem_privatekey) != 0) {
                        LOG_ERROR("%sFailed to KEM_DECODE_SHAREDSECRET. Worker error. Initiating graceful shutdown...", sio_ctx.worker.label);
                        sio_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
                    ...
                    sio_ctx.worker.hello1_ack_rcvd = true;
                    CLOSE_IPC_PROTOCOL(&received_protocol);
					continue;
				} else if (ircvdi.r_ipc_raw_protocol_t->type == IPC_MASTER_WORKER_HELLO2_ACK) {
                    if (!sio_ctx.worker.hello2_sent) {
                        LOG_ERROR("%sBelum pernah mengirim HELLO2", sio_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
                    if (sio_ctx.worker.hello2_ack_rcvd) {
                        LOG_ERROR("%sSudah ada HELLO2_ACK", sio_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
					ipc_protocol_t_status_t deserialized_ircvdi = ipc_deserialize(sio_ctx.worker.label,
...
//======================================================================
                    memcpy(sio_ctx.worker.remote_nonce, ihello2_acki->encrypted_wot_index, AES_NONCE_BYTES);
...
//----------------------------------------------------------------------  
                    ...
                    poly1305_init(&mac_ctx, sio_ctx.worker.kem_sharedsecret);
                    ...
                    if (!poly1305_verify(mac, data_mac)) {
                        LOG_ERROR("%sFailed to Mac Tidak Sesuai. Worker error. Initiating graceful shutdown...", sio_ctx.worker.label);
                        sio_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }            
...
//---------------------------------------------------------------------- 
                    ...
                    aes256ctx aes_ctx;
                    aes256_ctr_keyexp(&aes_ctx, sio_ctx.worker.kem_sharedsecret);
//=========================================IV===========================    
                    uint8_t keystream_buffer[sizeof(uint8_t) + sizeof(uint8_t)];
                    uint8_t iv[AES_IV_BYTES];
                    memcpy(iv, sio_ctx.worker.remote_nonce, AES_NONCE_BYTES);
...
//----------------------------------------------------------------------
                    ...
                    if (*(uint8_t *)&sio_ctx.worker.wot != *(uint8_t *)&data_wot) {
                        LOG_ERROR("%sberbeda wot. Worker error. Initiating graceful shutdown...", sio_ctx.worker.label);
                        sio_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
                    ...
                    if (sio_ctx.worker.idx != data_index) {
                        LOG_ERROR("%sberbeda index. Worker error. Initiating graceful shutdown...", sio_ctx.worker.label);
                        sio_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
...
//---------------------------------------------------------------------- 
                    if (async_create_incoming_event(sio_ctx.worker.label, &sio_ctx.worker.async, &sio_ctx.worker.heartbeat_timer_fd) != SUCCESS) {
                        LOG_ERROR("%sWorker error. Initiating graceful shutdown...", sio_ctx.worker.label);
                        sio_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
...
//---------------------------------------------------------------------- 
                    sio_ctx.worker.remote_ctr = (uint32_t)1;//sudah melakukan dekripsi data valid 1 kali
                    sio_ctx.worker.hello2_ack_rcvd = true;
//---------------------------------------------------------------------- 
                    CLOSE_IPC_PROTOCOL(&received_protocol);
					continue;
				} else {
