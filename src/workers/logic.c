...
void cleanup_logic_worker(worker_context_t *ctx) {
	cleanup_worker(ctx);
}

status_t setup_logic_worker(worker_context_t *ctx, worker_type_t *wot, uint8_t *index, int *master_uds_fd) {
    if (setup_worker(ctx, "Logic", wot, index, master_uds_fd) != SUCCESS) return FAILURE;
    return SUCCESS;
}

volatile sig_atomic_t logic_sigterm_requested = 0;

void logic_sigterm_handler(int sig) {
    logic_sigterm_requested = 1;
}
...
void run_logic_worker(worker_type_t *wot, uint8_t *index, double *initial_delay_ms, int *master_uds_fd) {
    ...
    if (setup_logic_worker(ctx, wot, index, master_uds_fd) != SUCCESS) goto exit;
    signal(SIGTERM, logic_sigterm_handler);
    while (!ctx->shutdown_requested && !logic_sigterm_requested) {
        ...
        for (int n = 0; n < snfds.r_int; ++n) {
            if (ctx->shutdown_requested || logic_sigterm_requested) {
				break;
			}
