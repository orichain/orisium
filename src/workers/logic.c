...
status_t setup_logic_worker(logic_context_t *logic_ctx, worker_type_t wot, int worker_idx, int master_uds_fd) {
    if (setup_worker(&logic_ctx->worker, "COW", wot, worker_idx, master_uds_fd) != SUCCESS) return FAILURE;
    return SUCCESS;
}
...
void run_logic_worker(worker_type_t wot, uint8_t worker_idx, long initial_delay_ms, int master_uds_fd) {
    ...
    if (setup_logic_worker(&logic_ctx, wot, worker_idx, master_uds_fd) != SUCCESS) goto exit;
...
//----------------------------------------------------------------------
                ...
                if (check_mac_ctr(
                        logic_ctx.worker.label, 
                        logic_ctx.worker.aes_key, 
                        logic_ctx.worker.mac_key, 
                        &logic_ctx.worker.remote_ctr, 
				...
				if (ircvdi.r_ipc_raw_protocol_t->type == IPC_MASTER_WORKER_INFO) {
                    ipc_protocol_t_status_t deserialized_ircvdi = ipc_deserialize(logic_ctx.worker.label,
                        logic_ctx.worker.aes_key, logic_ctx.worker.remote_nonce, &logic_ctx.worker.remote_ctr,
                        (uint8_t*)ircvdi.r_ipc_raw_protocol_t->recv_buffer, ircvdi.r_ipc_raw_protocol_t->n
                    );
                    if (deserialized_ircvdi.status != SUCCESS) {
                        LOG_ERROR("%sipc_deserialize gagal dengan status %d.", logic_ctx.worker.label, deserialized_ircvdi.status);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    } else {
                        LOG_DEBUG("%sipc_deserialize BERHASIL.", logic_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                    }           
                    ...
                    if (iinfoi->flag == IT_SHUTDOWN) {
                        ...
                        LOG_INFO("%sMaster Ready ...", logic_ctx.worker.label);
//----------------------------------------------------------------------
                        if (initial_delay_ms > 0) {
                            LOG_DEVEL_DEBUG("%sApplying initial delay of %ld ms...", logic_ctx.worker.label, initial_delay_ms);
                            sleep_ms(initial_delay_ms);
                        }
//----------------------------------------------------------------------
                        if (worker_master_hello1(&logic_ctx.worker) != SUCCESS) {
                            LOG_ERROR("%sWorker error. Initiating graceful shutdown...", logic_ctx.worker.label);
                            logic_ctx.worker.shutdown_requested = 1;
                            CLOSE_IPC_PROTOCOL(&received_protocol);
                            continue;
                        }
                    ...
                    memcpy(logic_ctx.worker.kem_ciphertext, ihello1_acki->kem_ciphertext, KEM_CIPHERTEXT_BYTES);
                    if (KEM_DECODE_SHAREDSECRET(logic_ctx.worker.kem_sharedsecret, logic_ctx.worker.kem_ciphertext, logic_ctx.worker.kem_privatekey) != 0) {
                        LOG_ERROR("%sFailed to KEM_DECODE_SHAREDSECRET. Worker error. Initiating graceful shutdown...", logic_ctx.worker.label);
                        logic_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
                    kdf1(logic_ctx.worker.kem_sharedsecret, logic_ctx.worker.aes_key);
                    kdf2(logic_ctx.worker.aes_key, logic_ctx.worker.mac_key);
                    if (worker_master_hello2(&logic_ctx.worker) != SUCCESS) {
                        LOG_ERROR("%sFailed to worker_master_hello2. Worker error. Initiating graceful shutdown...", logic_ctx.worker.label);
                        logic_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
...
//======================================================================
                    memcpy(logic_ctx.worker.remote_nonce, ihello2_acki->encrypted_wot_index, AES_NONCE_BYTES);
...
//----------------------------------------------------------------------  
                    ...
                    poly1305_init(&mac_ctx, logic_ctx.worker.kem_sharedsecret);
                    ...
                    if (!poly1305_verify(mac, data_mac)) {
                        LOG_ERROR("%sFailed to Mac Tidak Sesuai. Worker error. Initiating graceful shutdown...", logic_ctx.worker.label);
                        logic_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }            
...
//---------------------------------------------------------------------- 
                    ...
                    aes256ctx aes_ctx;
                    aes256_ctr_keyexp(&aes_ctx, logic_ctx.worker.kem_sharedsecret);
//=========================================IV===========================    
                    uint8_t keystream_buffer[sizeof(uint8_t) + sizeof(uint8_t)];
                    uint8_t iv[AES_IV_BYTES];
                    memcpy(iv, logic_ctx.worker.remote_nonce, AES_NONCE_BYTES);
...
//----------------------------------------------------------------------
                    ...
                    if (*(uint8_t *)&logic_ctx.worker.wot != *(uint8_t *)&data_wot) {
                        LOG_ERROR("%sberbeda wot. Worker error. Initiating graceful shutdown...", logic_ctx.worker.label);
                        logic_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
                    ...
                    if (logic_ctx.worker.idx != data_index) {
                        LOG_ERROR("%sberbeda index. Worker error. Initiating graceful shutdown...", logic_ctx.worker.label);
                        logic_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
...
//---------------------------------------------------------------------- 
                    if (async_create_incoming_event(logic_ctx.worker.label, &logic_ctx.worker.async, &logic_ctx.worker.heartbeat_timer_fd) != SUCCESS) {
                        LOG_ERROR("%sWorker error. Initiating graceful shutdown...", logic_ctx.worker.label);
                        logic_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
