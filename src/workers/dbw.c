...
//----------------------------------------------------------------------
                ...
                if (check_mac_ctr(
                        dbw_ctx.worker.label, 
                        dbw_ctx.worker.aes_key, 
                        dbw_ctx.worker.mac_key, 
                        &dbw_ctx.worker.remote_ctr, 
				...
				if (ircvdi.r_ipc_raw_protocol_t->type == IPC_MASTER_WORKER_INFO) {
                    ipc_protocol_t_status_t deserialized_ircvdi = ipc_deserialize(dbw_ctx.worker.label,
                        dbw_ctx.worker.aes_key, dbw_ctx.worker.remote_nonce, &dbw_ctx.worker.remote_ctr,
                        (uint8_t*)ircvdi.r_ipc_raw_protocol_t->recv_buffer, ircvdi.r_ipc_raw_protocol_t->n
                    );
                    if (deserialized_ircvdi.status != SUCCESS) {
                        LOG_ERROR("%sipc_deserialize gagal dengan status %d.", dbw_ctx.worker.label, deserialized_ircvdi.status);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    } else {
                        LOG_DEBUG("%sipc_deserialize BERHASIL.", dbw_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                    }           
...
//----------------------------------------------------------------------
                        if (worker_master_hello1(&dbw_ctx.worker) != SUCCESS) {
                            LOG_ERROR("%sWorker error. Initiating graceful shutdown...", dbw_ctx.worker.label);
                            dbw_ctx.worker.shutdown_requested = 1;
                            CLOSE_IPC_PROTOCOL(&received_protocol);
                            continue;
                        }
                    ...
                    if (!dbw_ctx.worker.hello1_sent) {
                        LOG_ERROR("%sBelum pernah mengirim HELLO1", dbw_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
                    if (dbw_ctx.worker.hello1_ack_rcvd) {
                        LOG_ERROR("%sSudah ada HELLO1_ACK", dbw_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
					ipc_protocol_t_status_t deserialized_ircvdi = ipc_deserialize(dbw_ctx.worker.label,
                    ...
                    memcpy(dbw_ctx.worker.kem_ciphertext, ihello1_acki->kem_ciphertext, KEM_CIPHERTEXT_BYTES);
                    if (KEM_DECODE_SHAREDSECRET(dbw_ctx.worker.kem_sharedsecret, dbw_ctx.worker.kem_ciphertext, dbw_ctx.worker.kem_privatekey) != 0) {
                        LOG_ERROR("%sFailed to KEM_DECODE_SHAREDSECRET. Worker error. Initiating graceful shutdown...", dbw_ctx.worker.label);
                        dbw_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
                    kdf1(dbw_ctx.worker.kem_sharedsecret, dbw_ctx.worker.aes_key);
                    kdf2(dbw_ctx.worker.aes_key, dbw_ctx.worker.mac_key);
                    if (worker_master_hello2(&dbw_ctx.worker) != SUCCESS) {
                        LOG_ERROR("%sFailed to worker_master_hello2. Worker error. Initiating graceful shutdown...", dbw_ctx.worker.label);
                        dbw_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
                    dbw_ctx.worker.hello1_ack_rcvd = true;
                    CLOSE_IPC_PROTOCOL(&received_protocol);
					continue;
				} else if (ircvdi.r_ipc_raw_protocol_t->type == IPC_MASTER_WORKER_HELLO2_ACK) {
                    if (!dbw_ctx.worker.hello2_sent) {
                        LOG_ERROR("%sBelum pernah mengirim HELLO2", dbw_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
                    if (dbw_ctx.worker.hello2_ack_rcvd) {
                        LOG_ERROR("%sSudah ada HELLO2_ACK", dbw_ctx.worker.label);
                        CLOSE_IPC_RAW_PROTOCOL(&ircvdi.r_ipc_raw_protocol_t);
                        continue;
                    }
					ipc_protocol_t_status_t deserialized_ircvdi = ipc_deserialize(dbw_ctx.worker.label,
...
//======================================================================
                    memcpy(dbw_ctx.worker.remote_nonce, ihello2_acki->encrypted_wot_index, AES_NONCE_BYTES);
...
//----------------------------------------------------------------------  
                    ...
                    poly1305_init(&mac_ctx, dbw_ctx.worker.kem_sharedsecret);
                    ...
                    if (!poly1305_verify(mac, data_mac)) {
                        LOG_ERROR("%sFailed to Mac Tidak Sesuai. Worker error. Initiating graceful shutdown...", dbw_ctx.worker.label);
                        dbw_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }            
...
//---------------------------------------------------------------------- 
                    ...
                    aes256ctx aes_ctx;
                    aes256_ctr_keyexp(&aes_ctx, dbw_ctx.worker.kem_sharedsecret);
//=========================================IV===========================    
                    uint8_t keystream_buffer[sizeof(uint8_t) + sizeof(uint8_t)];
                    uint8_t iv[AES_IV_BYTES];
                    memcpy(iv, dbw_ctx.worker.remote_nonce, AES_NONCE_BYTES);
...
//----------------------------------------------------------------------
                    ...
                    if (*(uint8_t *)&dbw_ctx.worker.wot != *(uint8_t *)&data_wot) {
                        LOG_ERROR("%sberbeda wot. Worker error. Initiating graceful shutdown...", dbw_ctx.worker.label);
                        dbw_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
                    ...
                    if (dbw_ctx.worker.idx != data_index) {
                        LOG_ERROR("%sberbeda index. Worker error. Initiating graceful shutdown...", dbw_ctx.worker.label);
                        dbw_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
                    }
...
//---------------------------------------------------------------------- 
                    if (async_create_incoming_event(dbw_ctx.worker.label, &dbw_ctx.worker.async, &dbw_ctx.worker.heartbeat_timer_fd) != SUCCESS) {
                        LOG_ERROR("%sWorker error. Initiating graceful shutdown...", dbw_ctx.worker.label);
                        dbw_ctx.worker.shutdown_requested = 1;
                        CLOSE_IPC_PROTOCOL(&received_protocol);
                        continue;
...
//---------------------------------------------------------------------- 
                    dbw_ctx.worker.remote_ctr = (uint32_t)1;//sudah melakukan dekripsi data valid 1 kali
                    dbw_ctx.worker.hello2_ack_rcvd = true;
//---------------------------------------------------------------------- 
                    CLOSE_IPC_PROTOCOL(&received_protocol);
					continue;
				} else {
