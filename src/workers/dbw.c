...
volatile sig_atomic_t dbw_sigterm_requested = 0;

void dbw_sigterm_handler(int sig) {
    dbw_sigterm_requested = 1;
}
...
void run_dbw_worker(worker_type_t *wot, uint8_t *index, double *initial_delay_ms, int *master_uds_fd) {
    ...
    signal(SIGTERM, dbw_sigterm_handler);
    while (!worker_ctx->shutdown_requested && !dbw_sigterm_requested) {
        ...
        for (int n = 0; n < snfds.r_int; ++n) {
            if (worker_ctx->shutdown_requested || dbw_sigterm_requested) {
				break;
			}
