...
static inline bool in_pool(const oritlsf_pool_t *pool, const void *p) {
    return (p && (uint8_t*)p >= pool->pool_start && (uint8_t*)p < pool->pool_end);
}
...
static inline void link_next_phys(const oritlsf_pool_t *pool, block_header_t *block) {
    ...
    if ((uint8_t*)next < pool->pool_end) {
        if (!in_pool(pool, next) || !header_aligned(next)) {
            return;
        }
...
static inline void tlsf_insert_block(oritlsf_pool_t *pool, block_header_t *block) {
    ...
    if (pool->free_lists[fli][sli]) {
        pool->free_lists[fli][sli]->prev_free = block;
    }
...
static inline block_header_t *tlsf_remove_block(oritlsf_pool_t *pool, block_header_t *block) {
    ...
    int fli, sli;
    get_indices(block->size, &fli, &sli);
    block_header_t **list_head = &pool->free_lists[fli][sli];
    if (block->prev_free) {
...
static inline void tlsf_add_pool(oritlsf_pool_t *pool, uint8_t *buffer, size_t size) {
    ...
    pool->pool_start = (uint8_t*)aligned_start;
    pool->pool_end = (uint8_t*)aligned_end;
    memset(pool->pool_start, 0, pool->pool_end - pool->pool_start);
    ...
    block_header_t *initial = (block_header_t*)pool->pool_start;
...
static inline block_header_t* find_suitable_block(oritlsf_pool_t *pool, size_t required, int req_fl, int req_sl, int *out_fl, int *out_sl) {
    for (int fl = req_fl; fl < FL_INDEX_COUNT; ++fl) {
        size_t sl_bitmap = pool->sl_bitmap[fl];
...
static inline void *oritlsf_malloc(oritlsf_pool_t *pool, size_t size) {
    if (pool->pool_start == NULL) return NULL;
    if (size == 0) size = 1;
    ...
    size_t candidate_fl = pool->fl_bitmap & (~((size_t)0) << req_fl);
    ...
    if (block_size >= required + MIN_BLOCK_TOTAL) {
        ...
        if (in_pool(pool, new_free) && ((uint8_t*)new_free + MIN_BLOCK_TOTAL <= pool->pool_end)) {
            ...
            if (new_size >= MIN_BLOCK_TOTAL) {
                ...
                if ((uint8_t*)next_phys < pool->pool_end) {
...
static inline void oritlsf_free(oritlsf_pool_t *pool, void **pptr) {
    ...
    if (prev_phys) {
        if (prev_phys->status == 0) {
            tlsf_remove_block(pool, prev_phys);
            coalesce = prev_phys;
...
static inline void *oritlsf_realloc(oritlsf_pool_t *pool, void *ptr, size_t newsize) {
    if (ptr == NULL) return oritlsf_malloc(pool, newsize);
    if (newsize == 0) {
...
//----------------------------------------------------------------------
    ...
    if (old_block_size >= required) {
        if (old_block_size >= required + MIN_BLOCK_TOTAL) {
            ...
            if ((uint8_t*)next_after_new < pool->pool_end) {
    ...
    if (in_pool(pool, next_phys) && header_aligned(next_phys) && next_phys->status == 0) {
    ...
    void *new_ptr = oritlsf_malloc(pool, newsize);
...
static inline void tlsf_dump_free_lists(const oritlsf_pool_t *pool) {
    ...
    for (int f=0; f<FL_INDEX_COUNT; ++f) {
        ...
        for (int s=0; s<SL_INDEX_COUNT; ++s) {
            ...
            while (cur) {
                if (!in_pool(pool, cur) || !header_aligned(cur) || cur->size > (size_t)(pool->pool_end-pool->pool_start) || cur->size < MIN_BLOCK_TOTAL) {
...
static inline void tlsf_validate_all(const oritlsf_pool_t *pool) {
    ...
    while (cur < pool->pool_end) {
        ...
        if (bh->size == OVERHEAD && (uint8_t*)bh + bh->size == pool->pool_end) {
            phy[phy_count++] = bh;
            break;
        }
        if (bh->size < MIN_BLOCK_TOTAL || bh->size > (size_t)(pool->pool_end - (uint8_t*)bh)) {
            fprintf(stderr, "tlsf_validate_all: Invalid block size at %p size=%zu\n", (void*)bh, bh->size);
            abort();
        }
        uint8_t *footer_loc = (uint8_t*)bh + bh->size - FOOTER_SIZE;
        if (!in_pool(pool, footer_loc) || footer_loc + FOOTER_SIZE > pool->pool_end) {
    ...
    for (int f = 0; f < FL_INDEX_COUNT; ++f) {
        for (int s = 0; s < SL_INDEX_COUNT; ++s) {
            ...
            while (node) {
                if (!in_pool(pool, node) || !header_aligned(node)) {
                ...
                if (node->size < MIN_BLOCK_TOTAL || node->size > (size_t)(pool->pool_end - pool->pool_start)) {
