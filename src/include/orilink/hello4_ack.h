#ifndef ORILINK_HELLO4_ACK_H
#define ORILINK_HELLO4_ACK_H
...
static inline status_t orilink_serialize_hello4_ack(const char *label, const orilink_hello4_ack_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
    ...
    memcpy(current_buffer + current_offset_local, payload->encrypted_remote_identity,
        sizeof(uint8_t) +
        sizeof(uint8_t) +
        sizeof(uint8_t) +
        sizeof(uint64_t) +
...
static inline status_t orilink_deserialize_hello4_ack(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_hello4_ack) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_hello4_ack_t *payload = p->payload.orilink_hello4_ack;
    if (current_offset + 
        sizeof(uint8_t) +
        sizeof(uint8_t) +
        sizeof(uint8_t) +
    ...
    {
        LOG_ERROR("%sOut of bounds reading encrypted_remote_identity.", label);
        return FAILURE_OOBUF;
    }
    memcpy(payload->encrypted_remote_identity, cursor,
        sizeof(uint8_t) +
        sizeof(uint8_t) +
        sizeof(uint8_t) +
        sizeof(uint64_t) +
...
static inline orilink_protocol_t_status_t orilink_prepare_cmd_hello4_ack(
    const char *label, 
    oritlsf_pool_t *pool, 
    uint8_t inc_ctr, 
...
)
{
	...
	result.r_orilink_protocol_t->type = ORILINK_HELLO4_ACK;
	orilink_hello4_ack_t *payload = (orilink_hello4_ack_t *)oritlsf_calloc(__FILE__, __LINE__, pool, 1, sizeof(orilink_hello4_ack_t));
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_hello4_ack_t payload. %s", label, strerror(errno));
		CLOSE_ORILINK_PROTOCOL(pool, &result.r_orilink_protocol_t);
		return result;
	}
    memcpy(payload->encrypted_remote_identity, encrypted_remote_identity, 
        sizeof(uint8_t) +
        sizeof(uint8_t) +
        sizeof(uint8_t) +
        sizeof(uint64_t) +
	...
	result.r_orilink_protocol_t->payload.orilink_hello4_ack = payload;
