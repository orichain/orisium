...
static inline status_t orilink_deserialize_hello3(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_hello3) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_hello3_t *payload = p->payload.orilink_hello3;
...
)
{
	...
	result.r_orilink_protocol_t->payload.orilink_hello3 = payload;
