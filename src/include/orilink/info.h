#ifndef ORILINK_INFO_H
#define ORILINK_INFO_H
...
static inline status_t orilink_serialize_info(const char *label, const orilink_info_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
static inline status_t orilink_deserialize_info(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_info) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_info_t *payload = p->payload.orilink_info;
...
static inline orilink_protocol_t_status_t orilink_prepare_cmd_info(
    const char *label, 
    oritlsf_pool_t *pool, 
    uint8_t inc_ctr, 
...
)
{
    ...
    payload->flag = flag;
	result.r_orilink_protocol_t->payload.orilink_info = payload;
