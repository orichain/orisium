...
static inline void cleanup_control_packet(const char *label, hierarchical_timer_wheel_t *timer, control_packet_t *h, bool clean_state, clean_data_type_t clean_data) {
    ...
    if (h->retry_timer_id.event) {
        if (h->retry_timer_id.event->prev_next_ptr != NULL) {
            htw_remove_event(timer, h->retry_timer_id.event);
        }
...
//----------------------------------------------------------------------
    if (single_session->heartbeat_sender_timer_id.event) {
        if (single_session->heartbeat_sender_timer_id.event->prev_next_ptr != NULL) {
            htw_remove_event(&ctx->timer, single_session->heartbeat_sender_timer_id.event);
        }
        single_session->heartbeat_sender_timer_id.event = NULL;
        single_session->heartbeat_sender_timer_id.id = 0ULL;
    }
    if (single_session->heartbeat_openner_timer_id.event) {
        if (single_session->heartbeat_openner_timer_id.event->prev_next_ptr != NULL) {
            htw_remove_event(&ctx->timer, single_session->heartbeat_openner_timer_id.event);
        }
        single_session->heartbeat_openner_timer_id.event = NULL;
