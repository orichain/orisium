...
static inline status_t ipc_serialize_worker_master_task_info(const char *label, const ipc_worker_master_task_info_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
static inline status_t ipc_deserialize_worker_master_task_info(const char *label, ipc_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.ipc_worker_master_task_info) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    ipc_worker_master_task_info_t *payload = p->payload.ipc_worker_master_task_info;
...
static inline ipc_protocol_t_status_t ipc_prepare_cmd_worker_master_task_info(const char *label, oritlsf_pool_t *pool, worker_type_t wot, uint8_t index, uint8_t session_index, task_info_type_t flag) {
	...
	result.r_ipc_protocol_t->type = IPC_WORKER_MASTER_TASK_INFO;
	...
	if (!payload) {
		LOG_ERROR("%sFailed to allocate ipc_worker_master_task_info_t payload. %s", label, strerror(errno));
		CLOSE_IPC_PROTOCOL(pool, &result.r_ipc_protocol_t);
		return result;
	}
    payload->session_index = session_index;
	payload->flag = flag;
	result.r_ipc_protocol_t->payload.ipc_worker_master_task_info = payload;
