...
static inline status_t ipc_serialize_udp_data(const char *label, const ipc_udp_data_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
static inline status_t ipc_deserialize_udp_data(const char *label, ipc_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
...
)
{
	...
	ipc_udp_data_t *payload = (ipc_udp_data_t *)oritlsf_calloc(__FILE__, __LINE__, pool, 1, sizeof(ipc_udp_data_t) + len);
	if (!payload) {
