...
status_t orilink_serialize_heartbeat_fin_ack(const char *label, const orilink_heartbeat_fin_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
status_t orilink_deserialize_heartbeat_fin_ack(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_heartbeat_fin_ack) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_heartbeat_fin_t *payload = p->payload.orilink_heartbeat_fin_ack;
...
orilink_protocol_t_status_t orilink_prepare_cmd_heartbeat_fin_ack(
    const char *label, 
    uint8_t inc_ctr, 
...
)
{
	...
	result.r_orilink_protocol_t->payload.orilink_heartbeat_fin_ack = payload;
