...
status_t orilink_deserialize_heartbeat_ack(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_heartbeat_ack) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
...
orilink_protocol_t_status_t orilink_prepare_cmd_heartbeat_ack(
    const char *label, 
    uint8_t inc_ctr, 
