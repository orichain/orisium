...
status_t orilink_serialize_statdt_ack(const char *label, const orilink_statdt_ack_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
status_t orilink_deserialize_statdt_ack(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_statdt_ack) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_statdt_ack_t *payload = p->payload.orilink_statdt_ack;
...
orilink_protocol_t_status_t orilink_prepare_cmd_statdt_ack(const char *label, uint64_t id, uint64_t sid, uint16_t arw, uint16_t data_len, uint8_t *data) {
	...
	result.r_orilink_protocol_t->type = ORILINK_STATDT_ACK;
	orilink_statdt_ack_t *payload = (orilink_statdt_ack_t *)calloc(1, sizeof(orilink_statdt_ack_t) + (2 * (data_len * sizeof(uint16_t))));
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_statdt_ack_t payload. %s", label, strerror(errno));
	...
	result.r_orilink_protocol_t->payload.orilink_statdt_ack = payload;
