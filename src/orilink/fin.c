...
status_t orilink_deserialize_fin(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_fin) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_fin_t *payload = p->payload.orilink_fin;
...
orilink_protocol_t_status_t orilink_prepare_cmd_fin(const char *label, uint64_t id) {
	...
	result.r_orilink_protocol_t->type = ORILINK_FIN;
	orilink_fin_t *payload = (orilink_fin_t *)calloc(1, sizeof(orilink_fin_t));
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_fin_t payload. %s", label, strerror(errno));
		CLOSE_ORILINK_PROTOCOL(&result.r_orilink_protocol_t);
		return result;
	}
    payload->id = id;
	result.r_orilink_protocol_t->payload.orilink_fin = payload;
