...
status_t orilink_deserialize_heartbeat_pong(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_heartbeat_pong) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_heartbeat_pong_t *payload = p->payload.orilink_heartbeat_pong;
...
orilink_protocol_t_status_t orilink_prepare_cmd_heartbeat_pong(const char *label, uint64_t id, uint64_t pid, uint8_t trycount) {
	...
	result.r_orilink_protocol_t->type = ORILINK_HEARTBEAT_PONG;
	orilink_heartbeat_pong_t *payload = (orilink_heartbeat_pong_t *)calloc(1, sizeof(orilink_heartbeat_pong_t));
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_heartbeat_pong_t payload. %s", label, strerror(errno));
