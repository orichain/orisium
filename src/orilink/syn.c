...
status_t orilink_serialize_syn(const char *label, const orilink_syn_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
    ...
    uint32_t pktnum_be = htobe32(payload->pktnum);
    memcpy(current_buffer + current_offset_local, &pktnum_be, sizeof(uint32_t));
    ...
    if (payload->mode > ORILINK_STREAMING) return FAILURE_IVLDMODE;
...
status_t orilink_deserialize_syn(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    ...
    if (current_offset + sizeof(uint32_t) > total_buffer_len) {
        LOG_ERROR("%sOut of bounds reading pktnum.", label);
        return FAILURE_OOBUF;
    }
    uint32_t pktnum_be;
    memcpy(&pktnum_be, cursor, sizeof(uint32_t));
    payload->pktnum = be32toh(pktnum_be);
