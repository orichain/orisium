...
status_t orilink_serialize_syndt_end(const char *label, const orilink_syndt_end_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
...
status_t orilink_deserialize_syndt_end(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_syndt_end) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_syndt_end_t *payload = p->payload.orilink_syndt_end;
...
orilink_protocol_t_status_t orilink_prepare_cmd_syndt_end(const char *label, uint64_t id, uint64_t sid, uint8_t trycount) {
	...
	result.r_orilink_protocol_t->type = ORILINK_SYNDT_END;
	orilink_syndt_end_t *payload = (orilink_syndt_end_t *)calloc(1, sizeof(orilink_syndt_end_t));
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_syndt_end_t payload. %s", label, strerror(errno));
	...
	result.r_orilink_protocol_t->payload.orilink_syndt_end = payload;
