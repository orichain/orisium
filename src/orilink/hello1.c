...
status_t orilink_serialize_hello1(const char *label, const orilink_hello1_t* payload, uint8_t* current_buffer, size_t buffer_size, size_t* offset) {
    ...
    memcpy(current_buffer + current_offset_local, payload->publickey1, PQCLEAN_MLKEM1024_CLEAN_CRYPTO_PUBLICKEYBYTES / 2);
...
status_t orilink_deserialize_hello1(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_hello1) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    ...
    if (current_offset + (PQCLEAN_MLKEM1024_CLEAN_CRYPTO_PUBLICKEYBYTES / 2) > total_buffer_len) {
        LOG_ERROR("%sOut of bounds reading publickey1.", label);
        return FAILURE_OOBUF;
    }
    memcpy(payload->publickey1, cursor, PQCLEAN_MLKEM1024_CLEAN_CRYPTO_PUBLICKEYBYTES / 2);
...
orilink_protocol_t_status_t orilink_prepare_cmd_hello1(const char *label, uint64_t client_id, uint8_t *publickey, uint8_t trycount) {
	...
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_hello1_t payload. %s", label, strerror(errno));
		CLOSE_ORILINK_PROTOCOL(&result.r_orilink_protocol_t);
		return result;
	}
    payload->client_id = client_id;
    memcpy(payload->publickey1, publickey, PQCLEAN_MLKEM1024_CLEAN_CRYPTO_PUBLICKEYBYTES / 2);
