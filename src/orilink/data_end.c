...
status_t orilink_deserialize_data_end(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_data_end) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_data_end_t *payload = p->payload.orilink_data_end;
...
orilink_protocol_t_status_t orilink_prepare_cmd_data_end(const char *label, uint64_t id, uint64_t sid, uint16_t spktnum, uint8_t trycount) {
	...
	result.r_orilink_protocol_t->type = ORILINK_DATA_END;
	orilink_data_end_t *payload = (orilink_data_end_t *)calloc(1, sizeof(orilink_data_end_t));
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_data_end_t payload. %s", label, strerror(errno));
	...
	result.r_orilink_protocol_t->payload.orilink_data_end = payload;
