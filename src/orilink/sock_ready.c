...
status_t orilink_deserialize_sock_ready(const char *label, orilink_protocol_t *p, const uint8_t *buffer, size_t total_buffer_len, size_t *offset_ptr) {
    if (!p || !buffer || !offset_ptr || !p->payload.orilink_sock_ready) {
        LOG_ERROR("%sInvalid input pointers.", label);
        return FAILURE;
    }
    size_t current_offset = *offset_ptr;
    const uint8_t *cursor = buffer + current_offset;
    orilink_sock_ready_t *payload = p->payload.orilink_sock_ready;
...
orilink_protocol_t_status_t orilink_prepare_cmd_sock_ready(const char *label, uint64_t client_id, uint8_t *server_id_port, uint8_t trycount) {
	...
	result.r_orilink_protocol_t->type = ORILINK_SOCK_READY;
	orilink_sock_ready_t *payload = (orilink_sock_ready_t *)calloc(1, sizeof(orilink_sock_ready_t));
	if (!payload) {
		LOG_ERROR("%sFailed to allocate orilink_sock_ready_t payload. %s", label, strerror(errno));
