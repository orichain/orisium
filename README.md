...
Orisium ensures long-term security against future quantum computing threats by building on $\text{NIST}$-standardized PQC primitives:

  * **PQC Key Encapsulation (KEM):** Uses **ML-KEM 1024** (NIST standard) for secure key exchange, providing an $\text{AES-256}$ equivalent security level.
  * **PQC Digital Signatures:** Uses **Falcon-512** (NIST standard) to verify message authenticity, guaranteeing strong, non-repudiable authentication.
  * **Strict Serial Handshake:** A rigid state machine (`HELLO` to `FINISH`) ensures every PQC key exchange step is verified and authenticated.
...
| :--- | :--- | :--- |
| **Control Stream** | **Serial & Reliable** (Requires $\text{ACK}$) | Establishes the $\text{PQC}$ session, performs **Network Orchestration**, and manages **Node Hierarchy**. Failure here triggers a controlled session disconnect. |
...
Security is maintained through a strict two-tiered cryptographic state:
  ...
  * **Stream State Synchronization ($\text{SYN\_DATA}$):** This frame communicates the **last verified $\text{Heartbeat}$ base counter** to the receiving endpoint, ensuring **every new data stream begins with a cryptographically unique state** (preventing $\text{Nonce}$ reuse).
...
The protocol includes a highly-secure mechanism to recover from extreme state drift:
  ...
  * **Temporary Rollback Check:** If a packet arrives with a non-matching counter, the protocol performs a temporary, non-destructive check. It copies the current counter ($\text{tmp\_ctr}$), decrements it, and **tests** if the packet validates against the previous state.
...
The protocol achieves superior resilience and efficiency through advanced adaptive logic:
  ...
  * **Adaptive Heartbeat (The Kalman Rocket):** Employs a **Kalman Filter** to predict network conditions based on accumulated **Retry Count** (reliability) and $\text{RTT}$ (performance). The heartbeat interval is dynamically adjusted: $\text{Interval} = \text{Base} \times 2^{\text{retry.prediction}}$.
  ...
  * **Two-Way State Synchronization:** The **Ping-Pong Heartbeat cycle** forces both $\text{SIO}$ and $\text{COW}$ endpoints into a mutual agreement on session timing and $\text{state}$. This eliminates *timer drift* and the need for frequent, costly rollback attempts.
...
### 5\. High-Throughput Data Optimization
  ...
  * **Selective Repeat ($\text{SR}$):** Each data packet operates with its own timer and is retransmitted independently, guaranteeing **maximum throughput** by ensuring only lost packets are resent.
  ...
  * **Safe Fragmentation:** Data is segmented into $\approx 1200 \text{ byte}$ fragments, mitigating the risks of $\text{IP}$ fragmentation and optimizing $\text{UDP}$ payload size.
...
The network is structured around $\mathbf{313}$ **Root Nodes** with deterministic recovery logic:

| Level | Role and Recovery Mechanism | Connectivity |
| :--- | :--- | :--- |
| **Root Nodes (313)** | The network core. When a $\text{Root}$ fails, its Level-1 nodes coordinate to promote a replacement via consensus. | $\mathbf{25}$ *downstreams* (Level-1) and $\mathbf{312}$ horizontal connections (partial mesh). |
| **Level-1 Nodes** | Connects to $\mathbf{1}$ *upstream Root* and $\mathbf{24}$ horizontal connections within the same *group*. | Maximum $\mathbf{25}$ *downstreams* (Level-2). |
| **Hierarchical Layers (L2 to L4)** | The structure continues with each node maintaining $\mathbf{1}$ *upstream*, $\mathbf{24}$ *horizontal*, and a maximum of $\mathbf{25}$ *downstreams*. | Ensures **geometric scalability** and short *multihop* paths. |
...
2.  **Optimized Path:** The $\text{Root}$ assigns the client to the most suitable $\text{upstream}$ (e.g., a Level-1 Node).
3.  **Persistence:** Nodes store the last verified $\text{upstream}$ information locally to ensure **session persistence**.
4.  **Automatic Fallback:** Upon failure, the node first attempts to reconnect to the last $\text{upstream}$; if this fails, it automatically **falls back to a Root Node** for a new route assignment.
...
Orisium is a foundational layer designed to solve critical scalability challenges in decentralized systems, particularly for blockchains and consensus mechanisms.
  ...
  * **Blockchain Performance:** $\text{Orisium}$ can be implemented to strengthen existing blockchain networks by:
      ...
      * **Enhancing Network Resilience:** Self-healing mechanisms improve resistance to $\text{DoS}$ attacks and partitioning.

-----

## 🛠️ Modular Architecture

Internal communication (`master` $\leftrightarrow$ `logic` $\leftrightarrow$ `cow`) is handled by **Unix Domain Sockets (UDS)** for maximum IPC speed and security, avoiding shared memory to eliminate *race conditions*.
...
| `cow` | 5 | *Outbound client* for horizontal and *upstream* connections (multiplexes $\sim65$ sessions per process). |
